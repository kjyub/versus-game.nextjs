...
export default function Error({ code, message, redirectUrl }: Props) {
  return (
    <div className="flex flex-center w-full h-screen">
      <div className="flex flex-col flex-center px-32 py-18 gap-8 rounded-4xl bg-black/40">
        ...
        {redirectUrl && (
          <Link href={redirectUrl}>
            <button className="px-8 py-4 text-xl font-medium text-white rounded-xl bg-black/30 hover:bg-black/40 transition-colors">
              홈으로 돌아가기
            </button>
          </Link>
        )}
