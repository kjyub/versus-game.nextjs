...
export default function VersusCommentPagination({pageIndex, pageSize, maxPage, maxPageButtons=5, itemCount, setPageIndex}) {
    ...
    return (
        <VS.CommentPaginationBox>
            {pageIndex > maxPageButtons && (
                <VS.CommentPaginationButton $is_active={false} onClick={()=>{handlePrev()}}>
                    <i className="fa-solid fa-backward"></i>
                </VS.CommentPaginationButton>
            )}
            {pages.map((page: number, index: number) => (
                <VS.CommentPaginationButton key={index} $is_active={page === pageIndex} onClick={()=>{setPageIndex(page)}}>
                    {page}
                </VS.CommentPaginationButton>
            ))}
            {pageIndex <= Math.floor(maxPage / maxPageButtons) * maxPageButtons && maxPage > maxPageButtons && (
                <VS.CommentPaginationButton $is_active={false} onClick={()=>{handleNext()}}>
